                        ...
                        {(fields, {add, remove}) => (
                            <>
                                {fields.map(({key, name, ...restField}) => {
                                    ...
                                    const showNetworkInfo = hasStorageNode(name); // 有存储节点就显示网卡信息
                                    // 从nodeStats状态中获取当前集群的统计信息
                                    const currentStats = nodeStats[name] || {storageCount: 0, clientCount: 0};
                                    // const { storageCount, clientCount } = getNodeStats(name);
                            ...
                            {favoriteConfigs.map((config, index) => (
                                ...
                                >
                                    ...
                                    <Space>
                                        <Button
                                            type="text"
                                            icon={config.isFavorite ? <StarFilled style={{color: '#faad14'}}/> :
                                                <StarOutlined/>}
                                            onClick={() => toggleFavorite(index)}
                                        />
                                        <Button
                                            type="link"
                                            onClick={() => applyTemplate(null, config.configData)}
                                        >
                                            应用
                                        </Button>
                                        <Button
